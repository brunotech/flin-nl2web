...
def run_evaluation(args, nsm_model, data_vec_dump):
    ...
    if not os.path.isdir(f'./all_results/{model_result_dict}/'):
        os.makedirs(f'./all_results/{model_result_dict}/')
    ...
    if args['eval_mode'] == 'of':
        ...
        result_basepath = f'./all_results/{model_result_dict}/'
        with open(f'{result_basepath}result_{str(test_trace_id)}.txt', 'w') as result_file:
            # read from a file
            result_DB = []
            ...
            for q_id_cnt, (q_inst, gt_list) in enumerate(test_query_DB.items(), start=1):
                ...
                reject = 0 if len(navigation_path[1]) == 2 else 1
                navigation_path_str = ','.join([str(element) for element in navigation_path[1]])
                print(f'Q-{q_id_cnt} of {total_queries} : {q_inst}')
                print(navigation_path_str)
                result_file.write(f'Q-{q_id_cnt}: {q_inst}' + '\n')
                result_file.write(f'gold path: {st_activity_name}, {str(gt_list)}' + '\n')
                result_file.write(f'Pred PATH: {navigation_path_str}' + '\n')
